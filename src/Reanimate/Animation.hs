...
-- Time in seconds, SVG represents the frame to pause at. Note, this is different that the
-- frame at the indicated time point.
newtype Slides = Slides [(Time, SVG)]

slidesEmpty :: Slides
slidesEmpty = Slides []

slidesAdjustOffset :: (Double -> Double) -> Slides -> Slides
slidesAdjustOffset fn (Slides lst) = Slides
  [ (fn t, d) | (t,d) <- lst ]

slidesConcat :: Slides -> Slides -> Slides
slidesConcat (Slides s1) (Slides s2) = Slides (s1++s2)

-- | Animations are SVGs over a finite time.
data Animation = Animation Slides Duration (Time -> SVG)

mkAnimation :: Duration -> (Time -> SVG) -> Animation
mkAnimation = Animation slidesEmpty

-- | Construct an animation with a duration of @1@.
animate :: (Time -> SVG) -> Animation
animate = Animation slidesEmpty 1
...
staticFrame d svg = Animation slidesEmpty d (const svg)

-- | Query the duration of an animation.
duration :: Animation -> Duration
duration (Animation _ d _) = d
...
seqA (Animation s1 d1 f1) (Animation s2 d2 f2) =
  Animation (slidesConcat s1 (slidesAdjustOffset (+d1) s2)) totalD $ \t ->
    if t < d1/totalD
      then f1 (t * totalD/d1)
...
parA (Animation _ d1 f1) (Animation _ d2 f2) =
  Animation slidesEmpty (max d1 d2) $ \t ->
    let t1 = t * totalD/d1
        t2 = t * totalD/d2 in
    mkGroup
    [ f1 (min 1 t1)
...
parLoopA (Animation _ d1 f1) (Animation _ d2 f2) =
  Animation slidesEmpty totalD $ \t ->
    let t1 = t * totalD/d1
        t2 = t * totalD/d2 in
    mkGroup
    [ f1 (t1 `mod'` 1)
...
parDropA (Animation _ d1 f1) (Animation _ d2 f2) =
  Animation slidesEmpty totalD $ \t ->
    let t1 = t * totalD/d1
        t2 = t * totalD/d2 in
    mkGroup
...
mapA fn = mapA' (const fn)

mapA' :: (Time -> SVG -> SVG) -> Animation -> Animation
mapA' fn (Animation s d f) = Animation s d (\t -> fn t (f t))
...
pauseAtBeginning t a =
    mkAnimation t (freezeFrame 0 a) `seqA` a
...
freezeFrame t (Animation _ d f) = const $ f (t/d)
...
adjustDuration fn (Animation _ d gen) =
  Animation slidesEmpty (fn d) gen
...
repeatA n (Animation _ d f) = Animation slidesEmpty (d*n) $ \t ->
  f ((t*n) `mod'` 1)
...
freezeAtPercentage frac (Animation s d genFrame) =
  Animation s d $ const $ genFrame frac
...
signalA fn (Animation s d gen) = Animation s d $ gen . fn
...
takeA len (Animation _ d gen) = Animation slidesEmpty len' $ \t ->
    gen (t * len'/d)
  where
    len' = clamp 0 d len
...
dropA len (Animation _ d gen) = Animation slidesEmpty len' $ \t ->
    gen (t * len'/d + len/d)
  where
    len' = d - clamp 0 d len
